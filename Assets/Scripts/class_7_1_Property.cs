...
namespace KID 
{
    ...
    public class class_7_1_Property : MonoBehaviour
    {
        ...
        // 唯讀屬性 攻擊值是50  Lambda是更精簡的寫法
        public int attack => 50;


        private void Start()
        {
            Debug.Log($"<color=#77f>血量hp：{hp}</color>");
            Debug.Log($"<color=#77f>修改前魔力mp：{mp}</color>");
            mp = 500;
            Debug.Log($"<color=#77f>修改後魔力mp：{mp}</color>");
        }
